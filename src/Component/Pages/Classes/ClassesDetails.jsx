  ...
  const bookingClass = id =>{
    ...
    return (
      ...
      <div className=" space-y-2 font-semibold">
        ...
        <h2>Price : ${price}</h2>
        {
         dis.find(i=>i===_id) || selected ? <><button disabled  className='py-2 px-5 bg-gray-500 text-white rounded-xl '>Select Now</button></>:<> <button onClick={()=>[bookingClass(_id),setSelected(!selected)]} className='py-2 px-5 bg-amber-500 text-white rounded-xl hover:bg-amber-400 transition duration-500'>Select Now</button></>
        }
