...
const ManageClass = () => {
  ...
  const updateRole = (id) => {
    axios.patch(`https://musicy-server-side.vercel.app/manageUsersRole/${id}`).then((res) => {
      refetch();
  ...
  const updateRoleDeny = (id) => {
    axios
      .patch(`https://musicy-server-side.vercel.app/manageUsersRoleDeny/${id}`)
      .then((res) => {
        refetch();
        if (res.data.modifiedCount > 0) {
          Swal.fire({
            ...
            <tbody>
              ...
              {addedClass.map((user, i) => (
                <tr key={i}>
                  ...
                  <td>
                    {user.status === "approved" || user.status === "deny" ? (
                      <>
                        <button
                          disabled
                  ...
                  <td>
                    {user.status === "approved" || user.status === "deny" ? (
                      <>
                        <button
                          disabled
                          onClick={() => updateRoleDeny(user._id)}
                  ...
                  <td>
                    <button className=" bg-amber-600 p-2 rounded">
                      <HiOutlineAnnotation className="w-4 h-4"></HiOutlineAnnotation>
                    </button>
                  </td>
                </tr>
              ))}
